$(function() {
	...
	var event = {
		...
		makeCenter: function() {
			$('#choose-box-wrapper-city').css("display", "block");
			$('#choose-box-wrapper-city').css("position", "absolute");
			$('#choose-box-wrapper-city').css("top", Math.max(0, (($(window).height() - $('#choose-box-wrapper-city').outerHeight()) / 2) + $(window).scrollTop()) + "px");
			$('#choose-box-wrapper-city').css("left", Math.max(0, (($(window).width() - $('#choose-box-wrapper-city').outerWidth()) / 2) + $(window).scrollLeft()) + "px");
		},
		...
		initTown: function(city) {
			...
			$('.town-item').bind('click', function() {
				...
				$('#town').val(item.text());
				$('#choose-box-wrapper-city').css("display", "none");
			});
		},
		select: function() {
